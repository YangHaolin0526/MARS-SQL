...
class LLMGenerationManager:
    ...
    def _process_next_obs(self, next_obs: List[str]) -> torch.Tensor:
        ...
        MAX_CHARS_FOR_TOKENIZER = 8192 

        # 检查传入的 next_obs 是否为字符串并且过长
        if isinstance(next_obs, str) and len(next_obs) > MAX_CHARS_FOR_TOKENIZER:
        
            # 打印一条警告日志，这样您就能在后台看到发生了什么
            print(f"\n[CRASH PREVENTION] Input to tokenizer was too long ({len(next_obs)} chars). "
                f"Truncating to {MAX_CHARS_FOR_TOKENIZER} chars to prevent regex bomb.\n")
        
            # 直接从开头截取字符串到安全长度
            # 这是因为对话历史（包含“炸弹”）通常在字符串的开头部分
            next_obs = next_obs[:MAX_CHARS_FOR_TOKENIZER]
    ...
    def execute_predictions(self, predictions: List[str], db_files: List[str], step: int, pad_token: str, active_mask=None, do_sql=True) -> List[str]:
        ...
        """
        # ========================= START: FIX FOR MULTI-TRAJECTORY =========================
        num_predictions = len(predictions)
        num_db_files = len(db_files)

        # If there are more predictions than db_files, it means multiple trajectories are used.
        # Expand the db_files list to match the number of predictions.
        if num_predictions > num_db_files and num_predictions % num_db_files == 0:
            num_trajectories = num_predictions // num_db_files
            # Repeat each db_file path for each trajectory
            expanded_db_files = [db for db in db_files for _ in range(num_trajectories)]
        else:
            # This handles the case where n_trajectories = 1 or if the numbers don't align.
            expanded_db_files = db_files
        # ========================== END: FIX FOR MULTI-TRAJECTORY ==========================
        ...
        # Use the expanded list here
        db_files_to_execute = [db_file for action, db_file in zip(cur_actions, expanded_db_files) if action == 'sql']
            ...
            if not active:
                ...
                if action == 'solution':
                    ...
                    if len(append_obs_str) > 1024:
                        ...
                        append_obs_str = f'\n\n<observation>Truncated to 10 lines since returned response too long: {res_str}\n{reminder_text}</observation>\n\n'
    ...
    def postprocess_predictions(self, predictions: List[Any]) -> Tuple[List[str], List[str]]:
        """
        Process (text-based) predictions from llm into actions and their content.
        This version handles both <sql>...</sql> and ```sql...``` formats.

        Args:
            predictions: List of raw predictions (strings) from the LLM.

        Returns:
            A tuple containing two lists:
            - actions: A list of actions ('sql', 'solution', or None).
            - contents: A list of the corresponding content (the SQL query or solution text).
        """
        actions = []
        contents = []

        # This pattern uses the '|' (OR) to match either the XML-style tag or the markdown code block.
        # It has two main parts:
        # 1. <(sql|solution)>(.*?)</\1> : Matches <sql>...</sql> or <solution>...</solution>.
        #    - Group 1: (sql|solution) - Captures the action type.
        #    - Group 2: (.*?) - Captures the content inside the tags.
        # 2. ```sql\s*(.*?)\s*``` : Matches markdown SQL blocks.
        #    - Group 3: (.*?) - Captures the content inside the backticks.
        pattern = r'<(sql|solution)>(.*?)</\1>|```sql\s*(.*?)\s*```'

        for prediction in predictions:
            action = None
            content = ''
            if isinstance(prediction, str):
                # Use re.DOTALL to make '.' match newlines, which is important for multi-line SQL queries
                match = re.search(pattern, prediction, re.DOTALL)
                if match:
                    # Check if the first part of the pattern (XML tag) was matched
                    if match.group(1):
                        action = match.group(1)  # 'sql' or 'solution'
                        content = match.group(2).strip()
                    # Otherwise, the second part (markdown block) must have matched
                    else:
                        action = 'sql'  # The action is implicitly 'sql' for this format
