...
def _pre_process_inputs(pad_token_id, prompt_token_ids) -> list[int]:
    """
    Pre-processes token IDs by removing left padding.
    This function is now robust and handles three cases:
    1. Input is a single number (int, float, etc.).
    2. Input is a sequence (list, 1D Tensor).
    3. Input is a sequence containing only padding tokens or is empty.
    """
    # Case 1: 如果输入是单个数字，直接将其包装在列表中返回。
    if isinstance(prompt_token_ids, numbers.Number):
        return [int(prompt_token_ids)]

    # Case 2: 如果输入是序列（list, Tensor, numpy array等），则进行处理。
    # 首先，确保它是一个 PyTorch Tensor。
    ...
    # 如果所有token都是填充token，也返回空列表。
    if non_pad_indices.numel() == 0:
        return []
    
    # 获取第一个非填充token的索引
    first_non_pad_index = non_pad_indices[0][0]
    
    # 从第一个非填充token开始，截取剩余部分并转换为列表
    token_ids = prompt_token_ids[first_non_pad_index:].tolist()
...
class Generator:
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        print(f"[DEBUG] In sqlact.py, generate_sequences, the 'prompts' object is: {prompts}")
        output = self.infer_engine.generate(
            prompt=prompts,  # because we have already convert it to prompt token id
...
class SQLActAgentGroup:
    def __init__(
        ...
        self.db_files = self._get_db_files(batch, db_path=gen_config.db_path)
    ...
    def _get_db_files(self, data, db_path):
        ...
        data_srcs = data.non_tensor_batch['data'] #train 用data_source, inference用data
        ...
        for db_id, data_src in zip(db_ids, data_srcs):
            ...
            elif data_src == 'spider2-lite':
                db_files.append(os.path.join(
                    "",
                    # Note: spider2-lite has a flatter directory structure
                    db_id + ".sqlite"
                ))
            elif data_src == 'sparc':
                db_files.append(os.path.join(
